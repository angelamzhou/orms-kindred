...
Everything a maintainer needs that a user does not: how the index is built and published, where each table comes from, what the ranking weights mean and how they were evaluated, and known gaps. For the design and evaluation write-up see `docs/kindred.pdf`; for the literature behind the weights see `docs/literature_review.md`.
...
|---|---|---|
| `data/authors.parquet` | `scripts/fetch_author_stats.py` (OpenAlex authors endpoint, 50 ids per call) | seniority penalty, early-career bonus, "works" column |
| `data/editors.csv` | `scripts/fetch_editorial_boards.py` (yearly Internet Archive snapshots of 14 journals' board pages; publishers block scripts; pace >= 8 s or archive.org rate-limits) | editorial penalty |
| `data/coi/genealogy.csv` | `scripts/fetch_genealogy.py NAME ...` (Mathematics Genealogy Project) or by hand | conflict flags |
| `data/references.parquet` | `scripts/normalize_works.py` (OpenAlex `referenced_works`, all outgoing references) | citation bonus in eval, OR-links for add-on authors |
| `data/departments.csv`, `data/rosters_raw/` | `scripts/fill_openalex_ids.py`, `scripts/scrape_rosters.py` | institution ids; faculty rosters (93 of 167 sites block scripts) |

## Abstract backfill

OpenAlex has no abstracts for most Elsevier papers. `scripts/backfill_abstracts.py` asks Semantic Scholar (500 DOIs per call;
set `S2_API_KEY` for a dedicated quota), skips Crossref for Elsevier (it has none) and, if `ELSEVIER_API_KEY` is set, tries the
ScienceDirect Article Retrieval API. `normalize_works.py` merges the results. Coverage: 62.5% of 74,127 papers; the remaining
gap is almost entirely Elsevier and needs the institutional key.

## Evaluation

`PYTHONPATH=src python -m kindred.eval -n 200 --cite-weight 0.1 [--lam ... -k ... --volume-correction ... --seniority-weight ...]`
runs leave-one-out: hold out a paper, use its embedding as the query and its OpenAlex references as the bibliography, rank all
authors, record where its true authors land. Current core index (29 venues, SPECTER2, cite 0.1): MRR 0.324, recall@10 0.327,
recall@20 0.388. Every knob that de-emphasises volume or seniority lowers this metric by construction; see `docs/kindred.pdf`.

## Learning weights

Ratings from the UI go to `~/.kindred/feedback.jsonl`; `kindred learn-weights [--save]` fits a pairwise logistic model toward
the current settings and writes `~/.kindred/weights.json`, which the UI uses as slider defaults.

## Known gaps

Elsevier abstracts (needs a key); OpenAlex author disambiguation; PhD students without an indexed paper (a preprint collection
would help); ML conference coverage in OpenAlex is about a third of the real proceedings; many roster and board pages block scripts.
